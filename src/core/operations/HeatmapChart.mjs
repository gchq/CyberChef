...
class HeatmapChart extends Operation {
    ...
    getHeatmapPacking(values, vBins, hBins) {
        ...
        if (xBounds[0] === xBounds[1])
            throw "Cannot pack points. There is no difference between the minimum and maximum X coordinate.";
        if (yBounds[0] === yBounds[1])
            throw "Cannot pack points. There is no difference between the minimum and maximum Y coordinate.";
        ...
        values.forEach((v) => {
            const fractionOfY =
                ...
                fractionOfX =
                    (v[0] - xBounds[0]) / (xBounds[1] + epsilon - xBounds[0]),
