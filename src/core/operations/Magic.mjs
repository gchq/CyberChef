    ...
    present(options) {
        ...
        options.forEach((option) => {
            ...
            const recipeConfig = currentRecipeConfig
                ...
                recipeURL =
                    "recipe=" +
                    Utils.encodeURIFragment(
                        Utils.generatePrettyRecipe(recipeConfig),
                    );
            ...
            let language = "",
                ...
                matchingOps = "",
                useful = "";
            const entropy = `<span data-toggle="tooltip" data-container="body" title="Shannon Entropy is measured from 0 to 8. High entropy suggests encrypted or compressed data. Normal text is usually around 3.5 to 5.">Entropy: <span style="color: ${chooseColour(
                    option.entropy,
                )}">${option.entropy.toFixed(2)}</span></span>`,
            ...
            if (option.languageScores[0].probability > 0) {
                ...
                if (likelyLangs.length < 1)
                    likelyLangs = [option.languageScores[0]];
                language =
            ...
            if (option.matchingOps.length) {
                matchingOps = `Matching ops: ${[
                    ...new Set(option.matchingOps.map((op) => op.op)),
                ].join(", ")}\n`;
            }
