        ...
        for (let amount = 1; amount < 26; amount++) {
            ...
            for (let i = 0; i < rotated.length; i++) {
                if (
                    ...
                    rotated[i] =
                        ((rotated[i] - lowerStart + amount) % 26) + lowerStart;
                } else if (
                    rotateUpper &&
                    ...
                    rotated[i] =
                        ((rotated[i] - upperStart + amount) % 26) + upperStart;
                } else if (
                    rotateNum &&
                    numStart <= rotated[i] &&
                    ...
                    rotated[i] =
                        ((rotated[i] - numStart + amount) % 10) + numStart;
                }
            }
            const rotatedString = Utils.byteArrayToUtf8(rotated);
            if (rotatedString.toLowerCase().indexOf(cribLower) >= 0) {
                const rotatedStringEscaped =
                    Utils.escapeWhitespace(rotatedString);
                if (printAmount) {
                    const amountStr =
