    ...
    run(input, args) {
        ...
        if (isWorkerEnvironment())
            self.sendStatusMessage(
                "Calculating " + Math.pow(256, keyLength) + " values...",
            );
        ...
        for (let key = 1, l = Math.pow(256, keyLength); key < l; key++) {
            ...
            if (printKey)
                record += "Key = " + Utils.hex(key, 2 * keyLength) + ": ";
            record += outputHex
                ? toHex(result)
                : Utils.escapeWhitespace(resultUtf8);
