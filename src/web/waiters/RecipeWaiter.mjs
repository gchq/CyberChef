...
class RecipeWaiter {
    ...
    initialiseOperationDragNDrop() {
        ...
        Sortable.create(recList, {
            ...
            setData: function (dataTransfer, dragEl) {
                dataTransfer.setData(
                    "Text",
                    dragEl.querySelector(".op-title").textContent,
                );
            },
            onEnd: function (evt) {
        ...
        // Favourites category
        document
            .querySelector("#categories a")
            .addEventListener("dragover", this.favDragover.bind(this));
        document
            .querySelector("#categories a")
            .addEventListener("dragleave", this.favDragleave.bind(this));
        document
            .querySelector("#categories a")
            .addEventListener("drop", this.favDrop.bind(this));
    }
    ...
    favDragover(e) {
        ...
        if (e.target?.className?.indexOf("category-title") > -1) {
            ...
            e.target?.parentNode?.className?.indexOf("category-title") > -1
        ) {
            // Hovering over the Edit button
            e.target.parentNode.classList.add("favourites-hover");
        } else if (
            e.target?.parentNode?.parentNode?.className?.indexOf(
                "category-title",
            ) > -1
        ) {
    ...
    favDragleave(e) {
        ...
        document
            .querySelector("#categories a")
            .classList.remove("favourites-hover");
    }
        ...
        for (let i = 0; i < operations.length; i++) {
            ...
            for (let j = 0; j < ingList.length; j++) {
                if (ingList[j].getAttribute("type") === "checkbox") {
                    ...
                    // toggleString
                    ingredients[j] = {
                        option: ingList[j].parentNode.parentNode.querySelector(
                            "button",
                        ).textContent,
    ...
    buildRecipeOperation(el) {
        ...
        const op = new HTMLOperation(
            opName,
            this.app.operations[opName],
            this.app,
            this.manager,
        );
        ...
        if (op.manualBake && this.app.autoBake_) {
            ...
            this.app.alert(
                "Auto-Bake is disabled by default when using this operation.",
                5000,
            );
        }
    }
    ...
    triggerArgEvents(op) {
        ...
        const triggerableOptions = op.querySelectorAll(
            ".populate-option, .arg-selector",
        );
    ...
    opAdd(e) {
        log.debug(
            `'${
                e.target.querySelector(".op-title").textContent
            }' added to recipe`,
        );
    ...
    setRegisters(opIndex, numPrevRegisters, registers) {
        const op = document.querySelector(
                `#rec-list .operation:nth-child(${opIndex + 1})`,
            ),
        ...
        for (let i = 0; i < registers.length; i++) {
            registerList.push(
                `$R${numPrevRegisters + i} = ${escapeControlChars(
                    Utils.escapeHtml(Utils.truncate(registers[i], 100)),
                )}`,
            );
        }
